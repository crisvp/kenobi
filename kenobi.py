...
# -*- coding: utf-8 -*-
#
# Gebaseerd op de cron.py uit de core ansible modules.
#
#
...
---
module: kenobi
short_description: Manage cron.d and crontab entries for kenobi.
description:
...
options:
  ...
  directory:
    description:
      - The directory in which to store the lockfile.
    required: false
    default: /var/run
  frequency:
    description:
      - The frequency with which to run the command, in minutes.
    required: false
    default: 1
  timeout:
    description:
      - The time before a lock is considered abandoned, in minutes.
    required: false
    default: 5
  kenobi_path:
    description:
      - Set the full path to the kenobi executable.
    required: false
    default: /usr/local/bin/kenobi
  kenobi_id:
    description:
...
class KenobiError(Exception):
    pass

class Kenobi(object):
    """
        Kenobi object to write time based crontab file
    ...
    def read(self):
        ...
        if self.cron_file:
            ...
            try:
                ...
                raise KenobiError("Unexpected error:", sys.exc_info()[0])
        else:
            # using safely quoted shell for now, but this really should be two non-shell calls instead.  FIXME
            (rc, out, err) = self.module.run_command(self._read_user_execute(), use_unsafe_shell=True)

            if rc != 0 and rc != 1: # 1 can mean that there are no jobs.
                raise KenobiError("Unable to read crontab")
    ...
    def write(self, backup_file=None):
        ...
        if HAS_SELINUX and self.cron_file:
    ...
    def get_cron_job(self,job,disabled):
        ...
        if self.cron_file:
            return "%s%s %s %s %s %s %s %s" % (disable_prefix,'*','*','*','*','*',self.user,job)
        else:
            return "%s%s %s %s %s %s %s" % (disable_prefix,'*','*','*','*','*',job)
...
def main():
    ...
    #
    # - kenobi: name="check dirs" hour="5,2" job="ls -alh > /dev/null"
    #
    # - name: do the job
    #   kenobi: name="do the job" hour="5,2" job="/some/dir/job.sh"
    #
    # - name: no job
    #   kenobi: name="an old job" state=absent
    #
    # - name: sets env
    #   kenobi: name="PATH" env=yes value="/bin:/usr/bin"
    #
    # Would produce:
    # PATH=/bin:/usr/bin
    # # Ansible: check dirs
    ...
    module = AnsibleModule(
        argument_spec = dict(
            ...
            directory=dict(default='/var/run'),
            timeout=dict(default=5, type='int'),
            frequency=dict(default=1, type='int'),
            disabled=dict(default=False, type='bool'),
            kenobi_path=dict(default='/usr/local/bin/kenobi'),
            kenobi_id=dict(default=uuid.uuid1()),
    ...
    kenobi_path  = module.params['kenobi_path']
    kenobi_id    = module.params['kenobi_id']
    ...
    os.umask(int('022', 8))
    crontab = Kenobi(module, user, cron_file)
    ...
    if env:
        ...
        if do_install:
            job = '%s -i "%s" -t %d -d "%s" -r %d -- %s' % (kenobi_path, kenobi_id, timeout, directory, frequency, job)

            job = crontab.get_cron_job(job, disabled)
            ...
            if len(old_job) > 0:
                comp_job = re.sub('-i "[^"]+"', 'ID', job)
                comp_oldjob = re.sub('-i "[^"]+"', 'ID', old_job[1])
                if comp_oldjob != comp_job:
